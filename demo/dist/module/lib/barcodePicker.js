...
export class BarcodePicker {
    constructor(originElement, { visible, singleImageMode, playSoundOnScan, vibrateOnScan, scanningPaused, guiStyle, videoFit, laserArea, viewfinderArea, scanner, scanSettings, targetScanningFPS, hideLogo }) {
    ...
    }, playSoundOnScan = false, vibrateOnScan = false, scanningPaused = false, guiStyle = BarcodePicker.GuiStyle.LASER, videoFit = BarcodePicker.ObjectFit.CONTAIN, laserArea, viewfinderArea, scanner, scanSettings = new ScanSettings(), enableCameraSwitcher = true, enableTorchToggle = true, enableTapToFocus = true, enablePinchToZoom = true, accessCamera = true, camera, cameraSettings, targetScanningFPS = 30 } = {}) {
    ...
    async processVideoFrame(highQualitySingleFrameMode) {
        ...
        if (!this.scanningPaused) {
            if (this.eventEmitter.listenerCount("submitFrame") > 0) {
                this.eventEmitter.emit("submitFrame", new ScanResult([], imageData.slice(), this.scanner.getImageSettings()));
            }
            try {
                const scanResult = await this.scanner.processImage(imageData, highQualitySingleFrameMode);
